        ...
        public override void Draw(GameTime gameTime, SpriteBatch _spriteBatch, Dictionary<string, SpriteFont> fonts, Dictionary<string, Texture2D> sprites) {
                    ...
                    if (!property.isOwned()) {
                        ...
                        if (property.isMortgaged) {
                            ...
                            if (property is PropertyResidential && propertyOwner != null && gamemanager.playerOwnsPropertySet(propertyOwner, (PropertyResidential)property)) {
                                _spriteBatch.DrawString(fonts["fontSmall"], "Set owned", gamemanager.spaces[i].position + new Vector2(200, 32), c);
                            }
                            ...
                            if (property is PropertyResidential) {
                                ...
                                if (propertyresidential.iHotelCount > 0) {
                                    _spriteBatch.DrawString(fonts["fontSmall"], string.Format("Hotels: {0}", propertyresidential.iHotelCount), gamemanager.spaces[i].position + new Vector2(250, 32), Color.Black);
                                }
                            }
            ...
            switch (gamemanager.gamestate) {
                ...
                case GameManager.GameState.LandOnSpace:
                    ...
                    _spriteBatch.DrawString(fonts["fontNormal"], "E: End Turn", new Vector2(32, 950), Color.Black);
                    break;

                case GameManager.GameState.GameOver:
                    _spriteBatch.DrawString(fonts["fontSmall"], "Game Over: Player " + gamemanager.playerCurrent.strName + " wins!", new Vector2(800, 750), Color.Black);
                    break;

            }

            _spriteBatch.DrawString(fonts["fontSmall"], gamemanager.strMessage, new Vector2(800, 700), Color.Black);
