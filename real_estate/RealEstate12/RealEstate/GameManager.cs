    ...
    public class GameManager {
        ...
        private void setup() {
            ...
            for (i = 0; i < 40; i++) {
                ...
                if (i == 10) {
                    ...
                    s = new SpaceEventCard(EventCard.EventCardType.MysteryVault);
                } else if (i == 7 || i == 22 || i == 36) {
                    s = new SpaceEventCard(EventCard.EventCardType.Happenstance);
                } else if (i == 0) {
                    s = new SpaceStart();
                } else if (i == 4) {
                    s = new SpaceTaxes(200);
                } else if (i == 38) {
                    s = new SpaceTaxes(75);
            ...
            for (i = 0; i < iPlayers; i++) {
                ...
                p.spacesMoved.Add(p.spaceCurrent);
                players.Add(p);
            }
            ...
            eventCards = new List<EventCard>[2];
            eventCards[(int) EventCard.EventCardType.Happenstance] = new List<EventCard>();
            eventCards[(int)EventCard.EventCardType.MysteryVault] = new List<EventCard>();

            EventCard eventcard;

            int iEventCardType = (int)EventCard.EventCardType.Happenstance;
            eventcard = new EventCard01();
            eventcard.gamemanager = this;
            eventCards[iEventCardType].Add(eventcard);

            eventcard = new EventCard02();
            eventcard.gamemanager = this;
            eventCards[iEventCardType].Add(eventcard);

            eventcard = new EventCard03();
            eventcard.gamemanager = this;
            eventCards[iEventCardType].Add(eventcard);



            iEventCardType = (int)EventCard.EventCardType.MysteryVault;
            eventcard = new EventCard17();
            eventcard.gamemanager = this;
            eventCards[iEventCardType].Add(eventcard);

            eventcard = new EventCard18();
            eventcard.gamemanager = this;
            eventCards[iEventCardType].Add(eventcard);

            eventcard = new EventCard19();
            eventcard.gamemanager = this;
            eventCards[iEventCardType].Add(eventcard);
        ...
        public void moveSpaces() {
            ...
            while (iSpaces > 0) {
                ...
                playerCurrent.spacesMoved.Add(playerCurrent.spaceCurrent);

                if (playerCurrent.spaceCurrent is SpaceStart) {
                    SpaceStart spacestart = (SpaceStart)playerCurrent.spaceCurrent;
                    spacestart.action();
                }
            ...
            if (playerCurrent.spaceCurrent is SpaceArrested) {
                incarceratePlayer(playerCurrent);
            } else if (playerCurrent.spaceCurrent is SpaceEventCard) {
                SpaceEventCard spaceeventcard = (SpaceEventCard)playerCurrent.spaceCurrent;
                if (spaceeventcard.eventcardtype == EventCard.EventCardType.Happenstance) {
                    int i = (int)EventCard.EventCardType.Happenstance;
                    EventCard eventCard = eventCards[i][0];
                    eventCard.action();
                    eventCards[i].Remove(eventCard);
                    eventCards[i].Insert(eventCards[i].Count, eventCard);

                } else if (spaceeventcard.eventcardtype == EventCard.EventCardType.MysteryVault) {
                    int i = (int)EventCard.EventCardType.MysteryVault;
                    EventCard eventCard = eventCards[i][0];
                    eventCard.action();
                    eventCards[i].Remove(eventCard);
                    eventCards[i].Insert(eventCards[i].Count, eventCard);
                }
            } else if (playerCurrent.spaceCurrent is SpaceTaxes) {
                SpaceTaxes spacetaxes = (SpaceTaxes)playerCurrent.spaceCurrent;
                spacetaxes.action();
            }
        }

        public void moveToSpace(Space spaceTarget) {
            int i;
            i = 0;
            while (playerCurrent.spaceCurrent != spaceTarget) {
                playerCurrent.spaceCurrent = playerCurrent.spaceCurrent.spaceNext;
                i++;
                if (i >= spaces.Count) {
                    break;
