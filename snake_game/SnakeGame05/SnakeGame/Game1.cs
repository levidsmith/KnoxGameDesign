...
namespace SnakeGame {
    public class Game1 : Game {
        ...
        private void DrawPreLevel() {
            ...
            int i;
            int iRowOffset;
            int iColOffset = (Arena.ARENA_COLS - 32) / 2;
            for (i = 0; i < 32; i++) {
                iRowOffset =  (Arena.ARENA_ROWS - 3) / 2;
                x = (i + iColOffset) * CELL_SIZE;
                y = iRowOffset * CELL_SIZE;
                _spriteBatch.Draw(sprCell, new Rectangle(x, y, CELL_SIZE, CELL_SIZE), Color.White);

                Color c;
                if (i == 0 || i == 31) {
                    c = Color.White;
                } else {
                    c = colorTextBackground;
                }

                    x = (i + iColOffset) * CELL_SIZE;
                iRowOffset++;
                y = iRowOffset * CELL_SIZE;
                _spriteBatch.Draw(sprCell, new Rectangle(x, y, CELL_SIZE, CELL_SIZE), c);
            ...
            iRowOffset = (Arena.ARENA_ROWS - 3) / 2;
            y = (iRowOffset + 2) * CELL_SIZE;
            _spriteBatch.DrawString(sprFont, string.Format(strText, gamemanager.iLevel), new Vector2(x, y), Color.White);
        ...
        private void DrawGameOver() {
            ...
            x = ((Arena.ARENA_COLS - strText.Length) / 2) * CELL_SIZE;
            iRowOffset = 6 + ((Arena.ARENA_ROWS - 3) / 2);
            y = (iRowOffset + 2) * CELL_SIZE;
            _spriteBatch.DrawString(sprFont, string.Format(strText, gamemanager.iLevel), new Vector2(x, y), Color.White);
