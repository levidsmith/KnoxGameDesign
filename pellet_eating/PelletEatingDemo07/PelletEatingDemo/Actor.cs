...
namespace PelletEatingDemo {
    public abstract class Actor {
        ...
        public bool canMoveDirection(Direction directionChange, List<Wall> walls) {
            if (directionChange == Direction.DOWN) {
                return !checkWallCollision(0, iSpeed, walls);
            } else if (directionChange == Direction.UP) {
                return !checkWallCollision(0, -iSpeed, walls);
            } else if (directionChange == Direction.LEFT) {
                return !checkWallCollision(-iSpeed, 0, walls);
