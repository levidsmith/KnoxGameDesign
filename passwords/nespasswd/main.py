...
async def game_query(name: str | None = None ):
  ...
  for key in game_data.keys():
    if name in game_data.get(key).get("name").lower():
      games.append(game_data.get(key))
...
@app.post("/game/NES-XU-USA/password")
async def name_password(query: mods.mega_man_3.QueryParams):
  return mods.mega_man_3.get_password(query)
...
#  game_data["NES-KI-USA"] = {"id": "NES-KI-USA", "name": "Kid Icarus" } 
  ...
  game_data["NES-XU-USA"] = {"id": "NES-XU-USA", "name": "Mega Man 3" } 
