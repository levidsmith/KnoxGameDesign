...
namespace Jumping {
    public class Game1 : Game {
        ...
        protected override void Draw(GameTime gameTime) {
            ...
            int iTextX = 8;
            int iTextY = 32;
            _spriteBatch.DrawString(fontRegular, string.Format("Constant Rise Velocity {0:0.00} blocks/seconds", Player.RISE_VELOCITY / BLOCK_SIZE), new Vector2(iTextX, iTextY), Color.White);

            iTextY += 40;
            _spriteBatch.DrawString(fontRegular, string.Format("Fall Acceleration {0:0.00} blocks/seconds^2", Player.FALL_ACCELERATION / BLOCK_SIZE), new Vector2(iTextX, iTextY), Color.White);
            ...
            iTextY += 40;
            _spriteBatch.DrawString(fontRegular, string.Format("Jump Time: {0:0.00}", player.fDebugJumpTime), new Vector2(iTextX, iTextY), Color.White);

            iTextY += 40;
            _spriteBatch.DrawString(fontRegular, string.Format("Jump Height: {0:0.00} px, {1:0.00} blocks", player.fDebugJumpHeight, player.fDebugJumpHeight / Game1.BLOCK_SIZE), new Vector2(iTextX, iTextY), Color.White);
