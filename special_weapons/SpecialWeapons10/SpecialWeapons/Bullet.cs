...
namespace SpecialWeapons {
    public class Bullet {
        ...
        public virtual void Draw(SpriteBatch sb, Dictionary<string, Texture2D> textures) {
            if (!isAlive) {
                return;
            }
        ...
        protected Enemy checkEnemyCollision(List<Enemy> listEnemies) {
            foreach (Enemy e in listEnemies) {
                if (e.isAlive && collided(e, (int)x, (int)y)) {
                    return e;
        ...
        protected Block checkBlockCollision(List<Block> listBlocks, int diff_x, int diff_y) {
        ...
        protected List<Block> getCollidedBlocks(List<Block> listBlocks, int diff_x, int diff_y) {
            List<Block> blocks;
